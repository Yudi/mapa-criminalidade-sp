...
export class GeocodingController {
  ...
  @ApiOperation({
    ...
    description:
      'Shares cached results and a single upstream request budget across users of this backend instance.',
  })
  ...
  @ApiResponse({
    status: 429,
    description: 'The shared provider request budget is busy; try again later.',
  })
  async search(
    ...
    if (this.active || Date.now() < this.nextRequestAt) {
      throw new HttpException(
        'Busca de endereços ocupada. Tente novamente em instantes.',
        429
      );
    }
    this.nextRequestAt = Date.now() + 1_000;
    const result = this.load(street, city, state)
      .then((value) => {
