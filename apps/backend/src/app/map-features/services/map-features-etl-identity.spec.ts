...
const describePostgis = process.env.TEST_DATABASE_URL ? describe : describe.skip;
  ...
  it('retains existing location IDs, removes vanished locations, and assigns new IDs', async () => {
    ...
    try {
      ...
      const feature = (hash: string): AggregatedFeature => ({
        num_bo: 'IDENTITY-REGRESSION', ano_bo: 2026, delegacia: null,
        latitude: -23.55, longitude: -46.63, location_hash: hash,
        category: 'Furto', rubrica_for_styling: 'Furto',
        data_ocorrencia: new Date('2026-03-01T00:00:00Z'), source_tables: [table],
        feature_data: {
          location: {}, occurrence: {}, all_rubricas: ['Furto'],
          records: [{ type: 'celular', source_table: table, source_id: 1 }],
          summary: { total_records: 1, celulares_count: 1, veiculos_count: 0,
            objetos_count: 0, dados_criminais_count: 0, produtividade_count: 0 },
        },
      });
      const db = {
      ...
      const service = new MapFeaturesEtlService(
        {} as PrismaService, {} as MapFeaturesQueryService
      ) as unknown as {
        removeSourceTableFeatures(table: string, db: DatabaseExecutor): Promise<void>;
        upsertFeatures(features: Map<string, AggregatedFeature>, db: DatabaseExecutor): Promise<number>;
      };
      const publish = async (hashes: string[]) => {
        await service.removeSourceTableFeatures(table, db);
        await service.upsertFeatures(new Map(hashes.map((hash) => [hash, feature(hash)])), db);
        const result = await client.query<{ id: string; location_hash: string }>(
          'SELECT id, location_hash FROM map_features WHERE num_bo = $1', ['IDENTITY-REGRESSION']
        );
