...
```bash
bun x prisma generate --schema path/to/schema.prisma
```

Do not run migrations without user confirmation, as they can cause data loss.

Do not edit database schema directly through queries without using Prisma Migrate, as it can lead to prisma errors and inconsistencies between the database and the Prisma schema.
