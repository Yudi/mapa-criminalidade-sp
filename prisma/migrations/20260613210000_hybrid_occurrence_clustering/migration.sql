...
BEGIN
  ...
  IF z < 16 THEN
    -- Low zoom sends compact aggregates. Only true singletons receive BO
    -- identity fields, making them directly clickable without copying the
    -- members of dense clusters into the tile.
    ...
    FROM (
      ...
      clusters AS (
        SELECT
          ...
          CASE WHEN count(*) = 1 THEN min(num_bo) END AS num_bo,
          CASE WHEN count(*) = 1 THEN min(ano_bo) END AS ano_bo,
          CASE WHEN count(*) = 1 THEN min(delegacia) END AS delegacia,
          ST_SetSRID(
            ST_MakePoint(
              avg(point_x),
              avg(point_y)
      ...
      SELECT
        ...
        delegacia,
        category,
        CASE WHEN cluster_count > 1 THEN cluster_count END AS cluster_count,
        CASE WHEN cluster_count > 1 THEN 1 END AS server_cluster,
        CASE WHEN cluster_count = 1 THEN 1 END AS server_singleton,
        ST_AsMVTGeom(cluster_geom, tile_envelope, 4096, 64, true) AS mvt_geom
      FROM clusters
    ) AS tile_data
    WHERE mvt_geom IS NOT NULL;
  ELSE
    -- At high zoom the frontend clusters only visible raw points. No
    -- aggregate marker fields are needed in these tiles.
    SELECT ST_AsMVT(tile_data, 'occurrences', 4096, 'mvt_geom')
    INTO mvt
    FROM (
      SELECT
